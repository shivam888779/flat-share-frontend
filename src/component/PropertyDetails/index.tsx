...
const PropertyDetails = (props: IPropertyDetailsProps) => {
    ...
    const { location, description, security, availableFrom, rent, deposit, mobile, isZeroDeposit, partnerGender, typeId, highLights, resources, preferences, images, occupancy } = propertyDetails;
    ...
    return <Box className="shadow-md rounded-lg p-8">
        ...
        <Stack direction={"row"} gap={8}>
            ...
            <Box my={3}>
                ...
                <Typography variant="subtitle2" ml={3.5}>  {occupancy??"NA"}</Typography>
            </Box>
            <Box my={3}>
                <Typography variant="h5" > <LocationCity /> Posted</Typography>
                <Typography variant="subtitle2" ml={3.5}>  {new Date(availableFrom).toLocaleDateString("en-GB", {
                    day: "2-digit",
                    month: "long",
                    year: "numeric"
                })}</Typography>
